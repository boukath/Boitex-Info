// lib/features/sections/new_livraison_page.dart
...
class _NewLivraisonPageState extends State<NewLivraisonPage> {
  ...
  Future<void> _saveSale() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      try {
        await _service.addLivraison(note);
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildTextField(_client, 'Client Name*', Icons.person_outline),
                      const SizedBox(height: 16),
                      _buildTextField(_phone, 'Phone Number', Icons.phone_outlined, keyboardType: TextInputType.phone),
                      const SizedBox(height: 16),
                      _buildTextField(_location, 'Location / Address', Icons.location_on_outlined),
                    ],
                  ),
                ),
              ),
                    ...
                    children: [
                      ...
                      Align(
                        ...
                        child: TextButton.icon(
                          ...
                          onPressed: () => setState(() => _lines.add(_LineItemControllers())),
                        ),
                      ),
                    ],
                  ),
        ...
        child: Card(
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                Expanded(flex: 4, child: _buildTextField(line.article, 'Article Name*', null)),
                const SizedBox(width: 8),
                Expanded(flex: 2, child: _buildTextField(line.qty, 'Qty*', null, keyboardType: TextInputType.number)),
                const SizedBox(width: 8),
                Expanded(flex: 3, child: _buildTextField(line.price, 'Price', null, keyboardType: TextInputType.number)),
                if (_lines.length > 1) IconButton(icon: const Icon(Icons.remove_circle_outline, color: Colors.redAccent), onPressed: () => setState(() => _lines.removeAt(i))) else const SizedBox(width: 48),
              ],
            ),
          ),
        ),
  ...
  TextFormField _buildTextField(TextEditingController controller, String label, IconData? icon, {TextInputType? keyboardType}) {
    return TextFormField(
      ...
      validator: (v) => (v == null || v.trim().isEmpty) && label.endsWith('*') ? 'Required' : null,
